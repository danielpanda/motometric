<script lang="ts">
  ...
  // Handlers
  function handleAddBike(name: string, type: BikeType, odometer: number, dailyUsage: number, freshService: boolean) {
...
<svelte:head>
  ...
  <meta name="description" content="Aplikasi pencatat servis motor dengan sistem dual trigger - gabungan KM dan waktu untuk notifikasi yang akurat dan personal." />
</svelte:head>
...
<div class="min-h-screen flex flex-col">
      ...
      {#if $garageCount === 0}
        ...
        <div class="text-center py-16 animate-fade-in">
          ...
          <p class="text-dark-500 mb-8 max-w-sm mx-auto">
            Catat servis motor kamu dengan sistem dual trigger - gabungan KM dan waktu untuk notifikasi yang akurat.
          </p>
          
          <button 
            onclick={() => showAddBikeModal = true}
          ...
          <div>
            ...
            <div class="space-y-3">
              {#each sortedStatuses as status (status.component.id)}
                <ComponentCard 
                  {status}
                  onService={() => handleServiceClick(status.component)}
                />
              {/each}
            </div>
          </div>
  ...
  <footer class="border-t border-dark-800/50 py-4 text-center text-xs text-dark-500">
    <p>MotoMetric © 2024 • Made with ❤️ for Indonesian Riders</p>
  </footer>
</div>
...
{#if selectedComponent && $activeBike}
  <ServiceModal 
    ...
    currentOdometer={$activeBike.currentOdometer}
    onClose={() => { showServiceModal = false; selectedComponent = null; }}
